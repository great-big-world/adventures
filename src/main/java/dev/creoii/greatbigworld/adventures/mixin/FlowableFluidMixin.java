...
public class FlowableFluidMixin {
    ...
    private boolean gbw$blockFluidFlowForWorldSize(boolean original, @Local(argsOnly = true) World world, @Local(ordinal = 1) BlockPos blockPos) {
        if (world.getLevelProperties() instanceof WorldSizeHolder worldSizeHolder && worldSizeHolder.gbw$getWorldSize() > 0) {
            return WorldSizeHolder.isWithinWorld(worldSizeHolder, blockPos);
        }
        return original;
    }
}
